...
def getValidActions(P_state):
   ...
   if P_state[112] == 2:
      Valid_Actions_return = np.full(321,0)
      arr_3 = P_state[:52]
...
def one_game(listAgent, perData):#----------------------------------------------
   env_state = initEnv()
   winner = -1
   while env_state[325] < 3000:
      pIdx = env_state[325] % 4
      # if turn != pIdx :
      #   turn = pIdx
      #   print("--------Turn =", env[113])
...
print(numba_main(numbaRandomBot, numbaRandomBot, numbaRandomBot, numbaRandomBot, 10000, perData))